...
const Products = () => {
  ...
  useEffect(() => {
    instance.get(`/products`, { params: { title, categoryId } }).then(res => setProducts(res.data)).finally(() => setLoading(false))
  }, [title, categoryId])
    ...
    <div className="p-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-7.5">
          ...
          <Select value={categoryId} setLoading={setLoading} setValue={setCategoryId} extraClass="!bg-slate-200 !text-black !w-[300px]" />
        </div>
        <Button onClick={() => navigate(PATH.productsCreate)} extraClass="!w-[100px]" type="button">Create</Button>
      </div>
      {loading ? <Loading /> : <ul className="flex justify-between gap-5 flex-wrap mt-5">
        {products.length ? products.map(item => <ProductCard key={item.id} item={item} />) : <FilterNotFound />}
      </ul>}
    </div>
  )
}
