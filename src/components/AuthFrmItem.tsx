...
const AuthFormItem:FC<AuthFormItemType> = ({label, type ,placeholder, name, extraClass, labelClass }) => {
    return(
        <label className={`${labelClass} mb-1.5 block text-sm text-slate-200` }>
            ...
            <input type={type} name={name} placeholder={placeholder}  className={`${extraClass} w-full rounded-2xl px-4 bg-slate-950/40 py-3 text-sm text-slate-100 placeholder:text-slate-500 ring-1 ring-white/10 outline-none transition focus:ring-2 focus:ring-indigo-400/60 `} />
        </label>
    )
}
