...
const ProductCard: FC<{ item: ProductsType }> = ({ item }) => {
    ...
    const erroImg = "https://cdni.iconscout.com/illustration/premium/thumb/sorry-item-not-found-illustration-svg-download-png-2809510.png"
    return (
        <div className="w-75 rounded-2xl overflow-hidden bg-slate-900/60 backdrop-blur-xl border border-white/10 shadow-xl transition hover:-translate-y-1 hover:shadow-2xl">
            <div className="h-50 relative overflow-hidden">
                <img onError={(evt) => (evt.target as HTMLImageElement).src = erroImg} src={item.images[0]} alt={item.title} className="w-full h-full object-cover transition duration-500 hover:scale-105" />
                ...
                <span onClick={() => navigate(PATH.productsMore)} className="absolute cursor-pointer top-3 right-3 text-xs p-1 rounded-full hover:bg-indigo-600 duration-300 bg-indigo-500/30 hover:text-white text-indigo-300 border border-indigo-400/20">
